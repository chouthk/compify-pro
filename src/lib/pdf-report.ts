...
function renderProfessionalReport(
  ...
  // Total row
  const totalScoreCalc = rubricItems.reduce((sum, r) => sum + r.score, 0);
  const totalMax = rubricItems.reduce((sum, r) => sum + r.max, 0);
  const totalH = 11;
  doc.setFillColor(240, 248, 255);
  doc.rect(margin, y, contentWidth, totalH, "F");
  doc.setDrawColor(...REPORT_PALETTE.line);
  doc.rect(margin, y, contentWidth, totalH, "S");
  doc.setFont(fontFamily, "bold");
  doc.setFontSize(8.5);
  doc.setTextColor(...REPORT_PALETTE.ink);
  doc.text("總分", margin + 3, y + 7);
  doc.setFontSize(10);
  doc.setTextColor(...REPORT_PALETTE.blue);
  doc.text(`${totalScoreCalc} / ${totalMax}`, margin + rCols[0] + rCols[1] / 2, y + 7.2, { align: "center" });
  const barX = margin + rCols[0] + rCols[1] + 3;
  const barW = rCols[2] - 6;
  doc.setFillColor(226, 232, 240);
  doc.roundedRect(barX, y + 4, barW, 3.4, 1.5, 1.5, "F");
  doc.setFillColor(...REPORT_PALETTE.blue);
  doc.roundedRect(barX, y + 4, Math.max(2, barW * (totalMax > 0 ? totalScoreCalc / totalMax : 0)), 3.4, 1.5, 1.5, "F");
  y += totalH + 6;
  ...
  y = 53;
  drawSectionHeading(doc, "四、優點", margin, y, fontFamily);
  y += 8;
  doc.setFont(fontFamily, "normal");
  doc.setFontSize(8);
  doc.setTextColor(...REPORT_PALETTE.muted);
  ...
  if (improvementsText) {
    ...
    y = 53;
    drawSectionHeading(doc, "五、改善空間", margin, y, fontFamily);
    y += 8;
  ...
  y = 53;
  drawSectionHeading(doc, "六、詳細分析", margin, y, fontFamily);
  y += 8;
  const analysisText =
  ...
  if (rewriting) {
    ...
    y = 53;
    drawSectionHeading(doc, "八、AI 改寫建議", margin, y, fontFamily);
    y += 8;
    renderMarkdownBlock(
      doc,
      rewriting,
  ...
  if (hasUpgrade) {
    ...
    let uy = 53;
    drawSectionHeading(doc, "九、5** 升級建議", margin, uy, fontFamily);
    uy += 9;
    doc.setFont(fontFamily, "normal");
    doc.setFontSize(8.5);
    doc.setTextColor(...REPORT_PALETTE.muted);
